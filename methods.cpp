...
void SynthesizeXRaysFromCT(NoduleSpecs nodule_spec, vector<vector<vector<Voxel *>>> ctVoxels,
    ...
    cout << "Source at:" << "(" << "," << source->y << "," << source->z << ")" << endl;
...
//    join all threads
    ...
    // string filename =  "textXRays/sp_" + to_string((int) source->x) + "_" + to_string((int) source->y) + "_" + to_string((int) source->z) + "_n" + ".txt";
    // string filename =  "textXRays/sp_" + to_string((int) source->x) + "_" + to_string((int) source->y) + "_" + to_string((int) source->z) + "_" + nodule_spec.filename ;
    string filename =  FILE + nodule_spec.filename ;
    cout << filename << endl;
...
void SynthesizeXRays(string ct_filename, string nodule_specs_filename, double voxel_xy_dim, double voxel_z_dim)
{
    ...
    vector<Coordinate *> source_pts = {
        ...
        cout<< "ctVoxels size: "<< ctVoxels.size()<<endl;
        NoduleSpecs nodule_spec;                                                                                                                                                                                                 
        nodule_spec.filename = nodule_specs_filename;  
        // SynthesizeXRaysFromCT(nodule_spec, ctVoxels, voxel_xy_dim, voxel_z_dim, source_pts[1]);
        // SynthesizeXRaysFromCT(nodule_spec, ctVoxels, voxel_xy_dim, voxel_z_dim, source_pts[6]);
        // SynthesizeXRaysFromCT(nodule_spec, ctVoxels, voxel_xy_dim, voxel_z_dim, source_pts[2]);
        SynthesizeXRaysFromCT(nodule_spec, ctVoxels, voxel_xy_dim, voxel_z_dim, source_pts[3]);
        // SynthesizeXRaysFromCT(nodule_spec, ctVoxels, voxel_xy_dim, voxel_z_dim, source_pts[4]);
        // SynthesizeXRaysFromCT(nodule_spec, ctVoxels, voxel_xy_dim, voxel_z_dim, source_pts[5]);
        // SynthesizeXRaysFromCT(nodule_spec,ctVoxels, voxel_xy_dim, voxel_z_dim, source_pts[6]);
...
void CalculateRemainingIntensity(SimulatedRay *ray, vector<vector<vector<Voxel *>>> &ctVoxels,
    ...
    // ray->remainingRay *= exp(-1 *power);   // added from ashish's file
    // cout << ray->remainingRay << endl;
    if (numberOfRaysFinished % 1000 == 0){

    // {   cout<< "----IMAGE_DIM---" << IMAGE_DIM<< endl;
    //     cout<< "----WATER_ATTENUATION---" << WATER_ATTENUATION <<endl;
